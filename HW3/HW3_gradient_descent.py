...
def gradient(X, Y):
    W = np.random.randn(9, 1)  # Initialize weight with random number
    MSE = np.zeros((EPOCH, 1))
    G = 0
    dW = 0
    for i in range(EPOCH):
        MSE[i] = np.power((Y - X @ W), 2).mean()  # Mean Squre Error
        G = -((Y-X@W).T @ X / X.shape[0]).T  # Gradient
        dW = -LR*G
        W += dW
...
def iteration(X, Y):
    """Iteration 100 times random weight to find best weight"""
    for i in range(100):  
        if i == 0:
            W, accuracy_W, MSE_W = gradient(X, Y)
        else:
            tmp, accuracy_tmp, MSE_tmp = gradient(X, Y)
    ...
    plt.figure()
    plt.plot(range(MSE_W.shape[0]), MSE_W)
    plt.title("MSE Loss")
    return W
...
if __name__ == '__main__':
    ...
    feature = normaliza(feature)
    target = np.concatenate((np.zeros((100,1)), np.ones((100,1))))
    weight = iteration(feature, target)
